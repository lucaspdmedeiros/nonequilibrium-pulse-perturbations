...
min_axis <- min(plot_df$expect_avg_growth_rate)
max_axis <- max(plot_df$expect_avg_growth_rate)
if (func_name == "rosenzweig_macarthur_2sp_forced_fixed_point") {
  axis_edge <- 0.41
...
if (func_name == "hastings_powell_3sp_forced_cycle") {
  axis_edge <- 0.08
  legend_breaks <- c(-0.08, -0.04, 0, 0.04, 0.08)
}
if (func_name == "hastings_powell_3sp_chaos") {
  axis_edge <- 0.13
...
if (n_sp == 2) {
  fig <- ggplot() +
    geom_point(data = plot_df, aes(x = x1, y = x2, color = expect_avg_growth_rate), 
               size = 5) +
